...
class single_encoder_base(pl.LightningModule):
    ...
    def forward(self,x_dict: dict):
        ...
        x = x_dict[self.encode_key] # bchw, [-1,1], either the first stage model output or the raw input
...
class edge_encoder_base(single_encoder_base):
    def __init__(self, *args, **kwargs):
        assert kwargs["encode_key"] in ['canny', 'mlsd', 'scribble', 'softedge']
        super().__init__(*args, **kwargs)
    
    def forward(self, x_dict: dict):
        x = super().forward(x_dict) # bchw, [-1,1], -1 for black, 1 for white
        #todo in development: there may be other operations for different edge types
        return x
